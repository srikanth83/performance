 ...
 */
class RegisteringNewUser extends Simulation {
  ...
  val registrationScenario = scenario("userRegistration").exec(homePage.homepage ,loginPage.register)



  setUp(registrationScenario.inject(rampUsers(perfTestUsers) over(perfTestSeconds)).protocols(config.Config.httpProtocol))
